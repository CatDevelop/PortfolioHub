<?php
	...
	else 
	{
		while($row = $A1->fetch_assoc()) {
			...
			$A3 = $Link->query("INSERT INTO LikedProjects (UserID, LikedUserID, ProjectID) VALUES('$UserID', '".$row["UserID"]."', '$ProjectID')"); 
	    }	
	}

	if($A2 && $A3)
		SendResponse($Link, ["message" => "Вы успешно поставили лайк!"]);
	else
