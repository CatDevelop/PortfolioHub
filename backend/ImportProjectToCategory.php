<?php
	...
	else 
	{
		while($row = $A1->fetch_assoc()) {
			...
			$b[$CategoryID]["projects"][] = $ProjectID;
			...
			$A3 = $Link->query("UPDATE `Projects` SET `InCategory`=1 WHERE `ID`=$ProjectID"); 
	    }	
	}

	if($A2)
		SendResponse($Link, ["message" => "Вы успешно импортировали проект!"]);
	else
